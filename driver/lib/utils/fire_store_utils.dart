...
class FireStoreUtils {
  ...
  static Future<List<VehicleModelModel>?> getVehicleModel(String brandId) async {
    ...
    await fireStore.collection(CollectionName.vehicleModel).where("isEnable", isEqualTo: true).where("brandId", isEqualTo: brandId).get().then((value) {
      for (var element in value.docs) {
  ...
  Stream<List<BookingModel>> getRejectedBookings() async* {
    ...
    Stream<QuerySnapshot> stream =
        fireStore.collection(CollectionName.bookings).where('rejectedDriverId', arrayContains: Constant.userModel!.id).orderBy("createAt", descending: true).snapshots();
    stream.listen((QuerySnapshot querySnapshot) {
      log("Length= : ${querySnapshot.docs.length}");
      bookingsList.clear();
      for (var document in querySnapshot.docs) {
  ...
  static Future<List<NotificationModel>?> getNotificationList() async {
    ...
    await fireStore.collection(CollectionName.notification).where('driverId', isEqualTo: FireStoreUtils.getCurrentUid()).orderBy('createdAt', descending: true).get().then((value) {
      for (var element in value.docs) {
  ...
  static Future<List<WithdrawModel>> getWithDrawRequest() async {
    ...
    await fireStore.collection(CollectionName.withdrawalHistory).where('driverId', isEqualTo: getCurrentUid()).orderBy('createdDate', descending: true).get().then((value) {
      for (var element in value.docs) {
  ...
  static Future<int> getTotalRide() async {
    final Query<Map<String, dynamic>> productList = FirebaseFirestore.instance.collection(CollectionName.bookings).where("driverId", isEqualTo: getCurrentUid());
  ...
  static Future<int> getRejectedRide() async {
    final Query<Map<String, dynamic>> productList = FirebaseFirestore.instance.collection(CollectionName.bookings).where("rejectedDriverId", arrayContains: getCurrentUid());
    AggregateQuerySnapshot query = await productList.count().get();
    log('The number of products: ${query.count}');
    return query.count ?? 0;
  }
